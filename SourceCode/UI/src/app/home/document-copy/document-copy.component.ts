...
export class DocumentCopyComponent extends BaseComponent implements OnInit {
  ...
  isLoading = false;
  constructor(
    public dialogRef: MatDialogRef<DocumentCopyComponent>,
  ...
  onCopied() {
    ...
    this.sub$.sink = this.homeService.copyDocument(copyDocumement)
      .subscribe((c: boolean) => {
  ...
  onPathClick(folder: Folder) {
    this.selectedFolder = this.cloneService.deepClone<Folder>(folder);
