...
/// 数据库上下文
/// </summary>
public class CaptureManagerContext : IdentityDbContext<User, Role, Guid, UserClaim, UserRole, IdentityUserLogin<Guid>, RoleClaim, IdentityUserToken<Guid>>
{
    public CaptureManagerContext(DbContextOptions<CaptureManagerContext> options) : base(options) { }
    ...
    public DbSet<UserDocumentClaim> DocumentUserPermissions { get; set; }
    ...
    public DbSet<DocumentComment> DocumentComments { get; private set; }
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        builder.Entity<Events>().ToTable("Events").HasKey(c => new { c.Id });
        builder.Entity<Rooms>().ToTable("Rooms").HasKey(c => new { c.Id });
        builder.Entity<Connections>().ToTable("Connections").HasKey(c => new { c.Id });

        builder.Entity<NLog>().ToTable("NLog").HasKey(c => new { c.Id });
        builder.Entity<LoginAudit>().ToTable("LoginAudits").HasKey(c => new { c.Id });


        builder.Entity<EmailTemplate>().ToTable("EmailTemplates").HasKey(c => new { c.Id });
        builder.Entity<EmailSMTPSetting>().ToTable("EmailSMTPSettings").HasKey(c => new { c.Id });
        builder.Entity<DocumentReminder>().ToTable("DocumentReminders").HasKey(c => new { c.Id });
        builder.Entity<DocumentStarred>().ToTable("DocumentStarreds").HasKey(c => new { c.DocumentId });
        builder.Entity<DocumentVersion>().ToTable("DocumentVersions").HasKey(c => new { c.Id });
        builder.Entity<DocumentAuditTrail>().ToTable("DocumentAuditTrails").HasKey(c => new { c.Id });
        builder.Entity<DocumentToken>().ToTable("DocumentTokens").HasKey(c => new { c.Id });
        builder.Entity<RecentActivity>().ToTable("RecentActivities").HasKey(c => new { c.Id });
        builder.Entity<UserNotification>().ToTable("UserNotifications").HasKey(c => new { c.Id });
        builder.Entity<DocumentShareableLink>().ToTable("DocumentShareableLinks").HasKey(c => new { c.Id });
        ...
        builder.Entity<VirtualFolderUser>()
          .HasKey(c => new { c.FolderId, c.UserId });
        ...
        {
            //entity.Property(u => u.SystemFolderName)
            //.Metadata
            //.SetAfterSaveBehavior(PropertySaveBehavior.Ignore);

            //entity.HasAlternateKey(x => x.SystemFolderName)
        ...
        {
            entity.ToTable("Documents").HasKey(c => new { c.Id });
