...
public partial class Frame : ContentControl
{
    ...
    public bool NavigateFromObject(object target, FrameNavigationOptions navOptions = null, UserControl mainView = null)
    {
        ...
        return NavigateCore(entry, NavigationMode.New, navOptions, mainView);
    }
    ...
    private bool NavigateCore(PageStackEntry entry, NavigationMode mode, FrameNavigationOptions options = null, UserControl mianview = null)
    {
        try
        {
            _isNavigating = true;
            ...
            if (mode == NavigationMode.New && !wasPageSet)
            {
                ...
                var page = CheckCacheAndGetPage(entry.SourcePageType);
                entry.Instance = page;
            }
            ...
            if (entry.Instance == null)
            {
                ...
                if (page is UserControl)
                {
                    ...
                    var vm = Locator.Current.GetService<IFrameNavigatedFrom>(name);
                    if (vm != null)
                    {
                        vm.Host = mianview;
                        page.DataContext = vm;
            ...
            else if (wasPageSet)
            {
                ...
                //传入时已经为我们创建了页面（NavigateFromObject路径）请立即尝试添加到缓存
                TryAddToCache(entry.Context, page);
            }
