...
public partial class Dialog : ContentControl, ICustomKeyboardNavigation
{
    public Dialog()
    {
    ...
    protected override void OnKeyUp(KeyEventArgs e)
    {
        ...
        switch (e.Key)
        {
            ...
            case Key.Enter:
                ...
                if (defButton != DialogButton.None)
                {
                    switch (defButton)
                    {
    ...
    /// </summary>
    public async Task<DialogResult> ShowAsync() => await ShowAsyncCore(null);

    /// <summary>
    /// Begins an asynchronous operation to show the dialog using the specified window
    /// </summary>
    public async Task<DialogResult> ShowAsync(Window w) => await ShowAsyncCore(w);

    /// <summary>
    /// Shows the content dialog on the specified window asynchronously.
    /// </summary>
    /// <remarks>
    /// Note that the placement parameter is not implemented and only accepts <see cref="DialogPlacement.Popup"/>
    /// </remarks>
    private async Task<DialogResult> ShowAsyncCore(Window window, DialogPlacement placement = DialogPlacement.Popup)
    {
        if (placement == DialogPlacement.InPlace)
    ...
    /// <summary>
    /// Closes the current <see cref="Dialog"/> without a result (<see cref="DialogResult"/>.<see cref="DialogResult.None"/>)
    /// </summary>
    public void Hide() => Hide(DialogResult.None);

    /// <summary>
    /// Closes the current <see cref="Dialog"/> with the given <see cref="DialogResult"/> <para>ddd</para>
    /// </summary>
    /// <param name="dialogResult">The <see cref="DialogResult"/> to return</param>
    ...
    /// </summary>
    protected virtual void OnPrimaryButtonClick(DialogButtonClickEventArgs args)
    {
    ...
    /// </summary>
    protected virtual void OnSecondaryButtonClick(DialogButtonClickEventArgs args)
    {
    ...
    /// </summary>
    protected virtual void OnCloseButtonClick(DialogButtonClickEventArgs args)
    {
        CloseButtonClick?.Invoke(this, args);
    }

    /// <summary>
    /// Called when the Dialog is requested to be opened
    /// </summary>
    ...
    /// <summary>
    /// Called after the Dialog is initialized but just before its presented on screen
    /// </summary>
    ...
    /// <summary>
    /// Called when the Dialog has been requested to close, but before it actually closes
    /// </summary>
    /// <param name="args"></param>
    protected virtual void OnClosing(DialogClosingEventArgs args)
    {
        Closing?.Invoke(this, args);
    }

    /// <summary>
    /// Called when the Dialog has been closed and removed from the tree
    /// </summary>
    protected virtual void OnClosed(DialogClosedEventArgs args)
    {
    ...
    private void HideCore()
    {
        ...
        var args = new DialogClosingEventArgs(_result);
    ...
    internal void SetupDialog()
    {
        ...
        bool setFocus = false;
        if (curFocus.FindAncestorOfType<Dialog>() == null)
        {
        ...
        switch (DefaultButton)
        {
...
#if DEBUG
                    Logger.TryGet(LogEventLevel.Debug, "Dialog")?.Log("SetupDialog", "Set initial focus to PrimaryButton");
#endif
                }
...
#if DEBUG
                    Logger.TryGet(LogEventLevel.Debug, "Dialog")?.Log("SetupDialog", "Set initial focus to SecondaryButton");
#endif
                }
            ...
            case DialogButton.Close:
                ...
                if (setFocus)
                {
                    _closeButton.Focus();
#if DEBUG
                    Logger.TryGet(LogEventLevel.Debug, "Dialog")?.Log("SetupDialog", "Set initial focus to CloseButton");
#endif
                }
...
#if DEBUG
                    Logger.TryGet(LogEventLevel.Debug, "Dialog")?.Log("SetupDialog", "Set initial focus to {next}", next);
#endif
                }
    ...
    // This is the exit point for the Dialog
    ...
    private async void FinalCloseDialog()
    {
        ...
        OnClosed(new DialogClosedEventArgs(_result));
