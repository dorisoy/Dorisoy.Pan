...
export class ObservableService {
  private _folderPath$: BehaviorSubject<string> = new BehaviorSubject<string>(
    null
  );
  private _toggle$: BehaviorSubject<string> = new BehaviorSubject<string>('');
  private _documentUploadProgress$: BehaviorSubject<FileProgress[]> =
    new BehaviorSubject<FileProgress[]>([]);
  private _documentFolderIds$: BehaviorSubject<DocumentFolderId[]> =
    new BehaviorSubject<DocumentFolderId[]>([]);
  private _documentFolderIds: DocumentFolderId[] = [];
  private _fileProcessList: FileProgress[] = [];
  public unCheckAllCheckBox: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(null);
  public progressBarOverlay: OverlayPanelRef<FileUploadProcessComponent>;

  public mainCheckBox$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false
  );

  private _selectedFolder$: BehaviorSubject<Folder> =
    new BehaviorSubject<Folder>(null);

  private _selectedDocument$: BehaviorSubject<Documents> =
    new BehaviorSubject<Documents>(null);

  private _checkAll$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false
  );
  ...
  private _rootFolder$: BehaviorSubject<Folder> = new BehaviorSubject<Folder>(
    null
  );
  ...
  setSelectedDocument(document: Documents) {
    this._selectedDocument$.next(
      this.cloneService.deepClone<Documents>(document)
    );
  }

  public upadteDocumentUploadProgress(
    fileName: string,
    percentage: number,
    isError = false,
    isComputeMd5 = false
    ...
    this._fileProcessList = this._fileProcessList.map((obj) =>
      fileProcess.id === obj.id ? fileProcess : obj
    );
  ...
  public setDocumentOrFolderId(dataObject: DocumentFolderId) {
    const isExist = this._documentFolderIds.find(
      (c) =>
        c.documentId == dataObject.documentId &&
        c.folderId == dataObject.folderId
    );
    if (isExist) {
      if (dataObject.documentId) {
        this._documentFolderIds = this._documentFolderIds.filter(
          (c) => c.documentId !== dataObject.documentId
        );
      } else {
        this._documentFolderIds = this._documentFolderIds.filter(
          (c) => c.folderId != dataObject.folderId
        );
      }
    } else {
    ...
    debugger;
    this._documentFolderIds$.next(
      this.cloneService.deepClone<DocumentFolderId[]>(this._documentFolderIds)
    );
  }
  ...
  public removeAll() {
    this._documentFolderIds$.next(
      this.cloneService.deepClone<DocumentFolderId[]>([])
    );
