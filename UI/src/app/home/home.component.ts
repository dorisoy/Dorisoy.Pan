...
export class HomeComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  deleteDocumentEvent(documentId: string) {
    this.selectedFolder.documents = this.selectedFolder.documents.filter(
      (c) => c.id != documentId
    );
  }
  ...
  onFolderUploadEvent(folder: Folder) {
    if (folder) {
      const existingFolder = this.selectedFolder.children.find(
        (c) => c.id == folder.id
      );
      if (!existingFolder) {
  ...
  onDoucmentUploadEvent(document: Documents) {
    ...
    let existingFolder = this.selectedFolder.documents.find(
      (c) => c.id == document.id
    );
    if (existingFolder) {
      this.selectedFolder.documents = this.selectedFolder.documents.map(
        (doc) => {
