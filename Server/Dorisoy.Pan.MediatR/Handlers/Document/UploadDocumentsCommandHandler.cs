...
namespace Dorisoy.Pan.MediatR.Handlers
{
    public class UploadDocumentsCommandHandler : IRequestHandler<UploadDocumentsCommand, ServiceResponse<DocumentDto>>
    {
        ...
        public async Task<ServiceResponse<DocumentDto>> Handle(UploadDocumentsCommand request, CancellationToken cancellationToken)
        {
            ...
            var folderPath = await _physicalFolderRepository.GetParentFolderPath(folder.PhysicalFolderId.Value);
            ...
            else
            {
                ...
                {
                    Id = documentId,
                    PhysicalFolderId = folder.PhysicalFolderId.Value,
                    Extension = extension,
            ...
            var createdByUser = await _userRepository.FindAsync(document.CreatedBy.Value);
            if (createdByUser != null)
            {
                documentUsers.Add(new UserInfoDto
                {
                    ...
                    RaleName = createdByUser.RaleName,
                    IsOwner = true
                });
            }
