...
namespace Dorisoy.Pan.Repository
{
    public class ConnectionMappingRepository : IConnectionMappingRepository
    {
        ...
        public void RemoveInId(Guid userId)
        {
            UserInfoToken userInfo;
            _onlineUser.TryRemove(userId, out userInfo);
        }
        ...
        public IEnumerable<UserInfoToken> GetAllUsers()
        {
        ...
        public UserInfoToken GetUserInfoByUserId(Guid uid)
        {
            foreach (var onlineUser in _onlineUser)
            {
                var user = onlineUser.Value;
